...
{
  /** @const */
  var fractionalUnit = Math.pow(10, this.digitsOfFractional);
  /** @const */
  var thousandUnit = Math.pow(10, this.digitsOfThousand);
  ...
  //handle decimal part
  var decimalString = this.padZero_(decimalPart, this.digitsOfFractional);
  ...
  while (intPart > 0)
  {
    ...
    if (intPart != intSegment)
      intSegment = this.padZero_(intSegment, this.digitsOfThousand);
  ...
  var displayString;
  if (ccySymbol)
    displayString = ccySymbol + currencyString;
  else
    displayString = currencyString + this.ccyCode;
 ...
 *  - Manage player account details<br>
 *  - Update commonUI to show play account detail<br>
 *  - Forge player display balance if game animation in progress<br>
 *
 * @constructor
 */
function Account() {
...
/**
 * This function is call by game at start-up phase, to initialize account setting
 * and set initial balance.
 * @param {Object} accountInfo
 *            An object contain account information, in the format of
 *            { ccy_code: 'GBP',
 ...
 *                'FREEBET': {display: '£10.00', code:'GBP', value: 10.00 ,
 *              currency_symbol: '£', ccy_thousand_separator: ',', ccy_decimal_separator: '.'},
 *            }.
 **/
Account.prototype.accountInit = function(accountInfo, balances) {
  if (!gcm.validate.isValidCurrencyOps(accountInfo)) {
    throw new Error('gcm.accountInit: Invalid ccy parameters ');
  }
  ...
  //init ccy formater
  this.ccyFormater_ = new CurrencyFormat(accountInfo['ccy_thousand_separator'],
    accountInfo['ccy_decimal_separator'],
    accountInfo['ccy_code']);
 ...
 * 'FREEBETS' and other balance type amounts at their previous level - only the cash balance will be updated.
 *
...
Account.prototype.balancesUpdate = function(balances, balanceFudge, changedFromCommonUI) {
  if (!gcm.validate.isBalances(balances)) {
 ...
 * @type {string}
 */
gcm.net.ws.ACCEPT_HEADER = 'application/xml;';
 ...
 * @param {Function} xmlResponseHandler function. *.*/
//TODO would it be better to add an error handler here, rather than throwing errors?
gcm.net.ws.getXmlResponse = function(url, xmlResponseHandlerObject, xmlResponseHandler)
{
  //setup http headers
  var headers = new goog.structs.Map();
  //headers.set('Accept',gcm.net.ws.ACCEPT_HEADER);

  goog.net.XhrIo.send(url, function(e) {
    var xhr = /** @type {goog.net.XhrIo} */ (e.target);
    if (xhr.isSuccess()) {
      //get xml response
      var xmlDoc = /** @type {Document} */ xhr.getResponseXml();
      //pass response doc to response handler
      xmlResponseHandler.call(xmlResponseHandlerObject, xmlDoc);
    } else if (xhr.getLastErrorCode() === goog.net.ErrorCode.ABORT) {
      throw new Error('com.openbet.gcm.net.ws.getXmlResponse: request to ' + url + ' aborted');
    } else if (xhr.getLastErrorCode() === goog.net.ErrorCode.TIMEOUT) {
      throw new Error('com.openbet.gcm.net.ws.getXmlResponse: request to ' + url + ' timed out');
    } else if (xhr.getLastErrorCode() === goog.net.ErrorCode.ACCESS_DENIED) {
      throw new Error('com.openbet.gcm.net.ws.getXmlResponse: request to ' + url + ' received access denied error');
    } else if (xhr.getLastErrorCode() === goog.net.ErrorCode.HTTP_ERROR) {
      if (xhr.getStatus() == 400) {
        //this is a bad request
        //we can retreive details from response body
        var xmlErrorDoc = /** @type {Document} */ xhr.getResponseXml();
        var errorMessage = xmlErrorDoc
                        .getElementsByTagName('ServiceError')[0]
                        .getElementsByTagName('Message')[0].childNodes[0].nodeValue;
        throw new Error('com.openbet.gcm.net.ws.getXmlResponse: bad request (400) to ' +
                         url + ', details : ' + errorMessage);
      } else if (xhr.getStatus() == 500) {
        throw new Error('com.openbet.gcm.net.ws.getXmlResponse: request to ' +
                         url + ', received internal server error, status code = 500 ');
      } else {
        throw new Error('com.openbet.gcm.net.ws.getXmlResponse: unknown error communicating with ' + url);
      }
    } else {
      throw new Error('com.openbet.gcm.net.ws.getXmlResponse: unknown error communicating with ' + url);
    }
  }, 'GET', null, headers, gcm.net.ws.TIMOUT_MILLIS);
...
//TODO change to goog style where we define functions as gcm.GameConfig = function...

/**
 * This class manage by gcm and aim to get game config from web service.
 *
 * @param {function()} readyCallback Function called when config is received from web service.
 * @param {string} webServiceUrl the base url for the game config web service, e.g. /gcm-ws/config.
 *                            it takes a errorObject when error accur.
 *                            <code>
 *                                    {
 *                                      errorCode: code,
 *                                      errorMessage: msg
 ...
 * @constructor
 */
function GameConfig(readyCallback, webServiceUrl) {
  this.readyCallback_ = readyCallback;
  this.webServiceUrlBase_ = webServiceUrl;
}
...
/**
 * the callback function to call with the config once
 * we have retrieved it
 * @type {function()}
 * @private
 */
GameConfig.prototype.readyCallback_;
...
/**
 * Handle a succesful response from gcm gameconfig web service We expect the
 * response to have the following format:<br>
 * TODO html encode the xml fragment
 * <pre>
 * <GameConfigResponse>
 *   <GameName>ChainReactors</GameName>
 *   <GameServerUrl>/fog/Servlet</GameServerUrl>
 *   <GameClass>Reactor</GameClass>
 * </GameConfigResponse>
 * </pre>
...
/**
 * @fileoverview FOG Error functions.
 * @author asugar
 */
goog.provide('gcm.fogerror');

//TODO perhaps move the errorCategory and errorSeverity out of here
//because these are no longer fog specific

/**
 * enumerate of error category
 **/
gcm.fogerror.errorCategory = {
 ...
 * */
gcm.fogerror.getErrorCategory = function(error) {
  switch (error) {
    case gcm.fogerror.errorCode.ERR_NO_LOGIN_COOKIE:
    case gcm.fogerror.errorCode.DECRYPT_LOGIN_COOKIE_FAILED_P1:
      return gcm.fogerror.errorCategory.LOGIN_ERROR;

    case gcm.fogerror.errorCode.INSUFFICIENT_FUNDS_P1:
      return gcm.fogerror.errorCategory.INSUFFICIENT_FUNDS;

    case gcm.fogerror.errorCode.JACKPOT_CHANGED_DUE_TO_WIN:
    case gcm.fogerror.errorCode.NUM_JACKPOTS_CHANGED:
    case gcm.fogerror.errorCode.STAKE_HIGHER_THAN_ALLOW_MAX:
    case gcm.fogerror.errorCode.STAKE_LOWER_THAN_ALLOW_MIN:
    case gcm.fogerror.errorCode.WARN_CUM_PLAY_COUNT_P1:
    case gcm.fogerror.errorCode.INSUFFICIENT_FUNDS_FREEPLAY:
    case gcm.fogerror.errorCode.ERR_SPLITTING_FRONT_HAND_BETTER:
      return gcm.fogerror.errorCategory.RECOVERABLE_ERROR;

    case gcm.fogerror.errorCode.CANNOT_CLOSE_GAME_IN_PROGRESS:
    case gcm.fogerror.errorCode.GUEST_CUTOFF_MSG:
    case gcm.fogerror.errorCode.FAILED_TO_FIND_GAME_DETAILS:
    case gcm.fogerror.errorCode.LOCKED_ACCOUNT:
    case gcm.fogerror.errorCode.MAX_WINNINGS_EXCEEDED:
    case gcm.fogerror.errorCode.REACHED_MAX_WINNINGS_P2:
    case gcm.fogerror.errorCode.REGION_BLOCKED:
    case gcm.fogerror.errorCode.REQ_GAME_NOT_FREE_PLAY:
      return gcm.fogerror.errorCategory.NON_RECOVERABLE_ERROR;
 ...
 * */
gcm.fogerror.getErrorSeverity = function(error) {
  switch (error) {
  case gcm.fogerror.errorCode.ERR_NO_LOGIN_COOKIE:
  case gcm.fogerror.errorCode.LOCKED_ACCOUNT:
  case gcm.fogerror.errorCode.DECRYPT_LOGIN_COOKIE_FAILED_P1:
  case gcm.fogerror.errorCode.GUEST_CUTOFF_MSG:
    return gcm.fogerror.errorSeverity.ERROR;
  ...
  case gcm.fogerror.errorCode.INSUFFICIENT_FUNDS_P1:
  case gcm.fogerror.errorCode.INSUFFICIENT_FUNDS_FREEPLAY:
    return gcm.fogerror.errorSeverity.INFO;
 ...
 * */
gcm.delegate.create = function(scope, func) {
  return function() {
    func.apply(scope);
  };
};
 ...
 *  - Categorize error by its error code and handle it accordingly
 *  - Call commonUI to show error message if needed
 *  - Resume game if is recoverable error, other wise reload game or return to lobby.
 *
 * @author X.Liu
 */
...
/**
 * This function is called by gcmCore to pass the commonUI instance
 * into error handling module
 * @param {Object} commonUI The instance of commonUI object kept by gcm.
 * */
ErrorHandler.prototype.setCommonUI = function(commonUI)
{
  this.commonUI_ = commonUI;
};
...
ErrorHandler.prototype.handleServerError = function(errorInfo) {
  ...
  errorCategory = gcm.fogerror.getErrorCategory(errorCode);
  // figure out the error type
  errorSeverity = gcm.fogerror.getErrorSeverity(errorCode);

  //ask the commonUI to handle it (display and decide what to do after displaying)
  this.commonUI_.handleError(errorCategory, errorSeverity, errorCode, errorMSG);
 ...
 */
ErrorHandler.prototype.handleError = function(errorCategory, errorSeverity, errorCode, errorMessage)
{
  //validation
  if (!gcm.validate.isEnumOption(gcm.fogerror.errorCategory, errorCategory)) {
    throw new Error('gcm.handleError: Invalid errorCategory');
  }
  if (!gcm.validate.isEnumOption(gcm.fogerror.errorSeverity, errorSeverity)) {
    throw new Error('gcm.handleError: Invalid errorSeverity');
  }
  if (!goog.isDefAndNotNull(errorCode)) {
    throw new Error('gcm.handleError: Please specify an errorMessage');
  }
  if (!goog.isDefAndNotNull(errorMessage)) {
    throw new Error('gcm.handleError: Please specify an errorMessage');
  }

  this.commonUI_.handleError(errorCategory, errorSeverity, errorCode, errorMessage);
};



goog.provide('gcm.gamecontrol.GamePhaseController');
goog.require('gcm.validate');

/**
 *
 * This GamePhaseController is a singleton class for managing game phases and coordinating
 * phase actions between game and commonUI.
 *
 * @author xliu
 * Date: 17/07/12
 * Time: 14:45
 * @namespace
 */
var GamePhaseController = {};


/**
 * The possible life-cycle phases that the system can be in:
 * COMMONUI_NOTIFY, GAME_ANIMATING and IDLE.
 * @enum {string}
 */
GamePhaseController.PlayPhases = {
...
/**
 * The life-cycle phase the system is in
 * @type {GamePhaseController.PlayPhases}
 * @private
 */
GamePhaseController.playPhase_ = GamePhaseController.PlayPhases.IDLE;

/**
 * @private
 * @type {Object}
 * */
GamePhaseController.commonUI_ = null;
 ...
 * */
GamePhaseController.init = function(commonUI)
{
  GamePhaseController.commonUI_ = commonUI;
};
 ...
 */
GamePhaseController.gameAnimationStart = function() {
  GamePhaseController.playPhase_ = GamePhaseController.PlayPhases.GAME_ANIMATING;
  if (GamePhaseController.commonUI_) {
    GamePhaseController.commonUI_.gameAnimationStart();
  }
};
 ...
 */
GamePhaseController.gameAnimationComplete = function(resumeCallback) {

  if (!gcm.validate.isFunction(resumeCallback)) {
    throw new Error('gcm.gameAnimationComplete: Invalid callback function');
  }

  GamePhaseController.playPhase_ = GamePhaseController.PlayPhases.COMMONUI_NOTIFY;
  // This is where notifications will be dealt with when implemented
  // For now we just call the resumeCallback immediately to tell the
  // game that it can resume
  GamePhaseController.playPhase_ = GamePhaseController.PlayPhases.IDLE;
  resumeCallback();
  if (GamePhaseController.commonUI_) {
    ...
    GamePhaseController.commonUI_.gameAnimationComplete();
  }
};
 ...
 * @return {boolean} the initial value of the option is returned back to the
...
GameInfo.regOption = function(optionType) {
  ...
  switch (optionType)
  {
    case GameInfo.OptionTypes.MUTE:
      // mute option should default to true
      // note that in the future we can persist default options as cookies or
      // against the database
      GameInfo.options_[optionType] = true;
      break;
 ...
 * Time: 13:43
 */
goog.provide('gcm.config.GameSession');
goog.require('gcm.net.ws');
 ...
 * @param {Function} sessionTimerReadyCallback Function called when session timer is received from web service.
 * @param {Function} sessionStatsReadyCallback Function called when session acknowledgement
 *                                               is received from web service.
 * @constructor
 */
function GameSession(webServiceUrl, sessionTimerReadyCallback, sessionStatsReadyCallback) {
  this.sessionWSUrlBase_ = webServiceUrl;
  this.timerReadyCallback = sessionTimerReadyCallback;
  this.statsReadyCallback = sessionStatsReadyCallback;

  this.timerRequestInterval_ = null;
  this.ackDue_ = -1;
}
 ...
 * @type {number}
 * */
GameSession.TIMER_REQUEST_INTERVAL = 300;
...
GameSession.prototype.run = function()
{
  ...
  this.requestSessionTimer();

  //Start session timer request interval
  //Intervals using second as time unit thus *1e3
  this.timerRequestInterval_ = setInterval(gcm.delegate.create(this, this.requestSessionTimer),
    GameSession.TIMER_REQUEST_INTERVAL * 1e3);

  //start seesion ack update interval
  setInterval(gcm.delegate.create(this, this.localSessionAckUpdate), GameSession.ACK_UPDATE_INTERVAL * 1e3);
};

/**
 * This function will be called by console when user choose to quit current session.
 * */
GameSession.prototype.requestSessionqQuit = function()
{
  //gcm.net.ws.getXmlResponse(this.sessionWSUrlBase_ + 'continue', null, null);
};

/**
 * This function will be called by console when user choose to continue current session.
 * */
GameSession.prototype.requestSessionContinue = function()
{
  gcm.net.ws.getXmlResponse(this.sessionWSUrlBase_ + 'continue', this, this.sessionContinueResponseHandler);
};

/**
 * @private
 * */
GameSession.prototype.localSessionAckUpdate = function()
{
  this.ackDue_ -= GameSession.ACK_UPDATE_INTERVAL;

  if(this.ackDue_ <= 0)
    gcm.net.ws.getXmlResponse(this.sessionWSUrlBase_ + 'stats', this, this.sessionStatsResponseHandler);
};

/**
 * @private
 * */
GameSession.prototype.requestSessionTimer = function()
{
  gcm.net.ws.getXmlResponse(this.sessionWSUrlBase_ + 'timer', this, this.sessionTimerResponseHandler);
};
 ...
 * @private
 * */
GameSession.prototype.sessionTimerResponseHandler = function(xmlDocResponse)
{
  ...
  try {
    ...
    throw new Error('com.openbet.gcm.config.GameSession.sessionTimerResponseHandler: failed to parse session response');
  }

  this.timerReadyCallback.call(window, duration);
};
 ...
 * @private
 * */
GameSession.prototype.sessionContinueResponseHandler = function(xmlDocResponse)
{
  ...
  try {
    ...
    throw new Error('com.openbet.gcm.config.GameSession.sessionContinueResponseHandler: failed to parse session response');
  }
  ...
  if('true' == sessionExtended)
  {
    ...
    this.ackDue_ = GameSession.TIMER_REQUEST_INTERVAL * 10;
  }
};
 ...
 * @private
 * */
GameSession.prototype.sessionStatsResponseHandler = function(xmlDocResponse)
{
  var statsNode, stakes, winnings, turnover;

  try {
    statsNode = xmlDocResponse.getElementsByTagName('SessionStatsResponse')[0];
    stakes = parseInt(statsNode.getElementsByTagName('Stakes')[0].childNodes[0].nodeValue, 10);
    winnings = parseInt(statsNode.getElementsByTagName('Winnings')[0].childNodes[0].nodeValue, 10);
    turnover = parseInt(statsNode.getElementsByTagName('Turnover')[0].childNodes[0].nodeValue, 10);
  } catch (e) {
    throw new Error('com.openbet.gcm.config.GameSession.sessionStatsResponseHandler: failed to parse session response');
  }

  this.statsReadyCallback.call(window, stakes, winnings, turnover);
};
goog.provide('gcm');
goog.require('gcm.Account');
goog.require('gcm.ErrorHandler');
goog.require('gcm.config.GameConfig');
goog.require('gcm.config.GameSession');
goog.require('gcm.fogerror');
goog.require('gcm.gamecontrol.GameInfo');
goog.require('gcm.gamecontrol.GamePhaseController');
...
gcmCore.commonUIReady = function(commonUI) {
  ...
  //set commonUI for gcm error handler
  gcmCore.errorHandler_.setCommonUI(gcmCore.commonUI_);

  //set commonUI for game information module
  GameInfo.setCommonUI(gcmCore.commonUI_);

  //init GamePhaseController
  GamePhaseController.init(gcmCore.commonUI_);
...
/**
 * called when session timer is ready
 * @param {Number} duration The duration on current session.
 * @private
 */
gcmCore.sessionTimerReady_ = function(duration) {
  gcmCore.commonUI_.sessionDurationUpdate(duration);
};

/**
 * called when session stats is ready.
 ...
 * @private
 */
gcmCore.sessionStatsReady_ = function(stakes, winnings, turnover) {
  gcmCore.commonUI_.sessionStatsDisplay(stakes, winnings, turnover);
};
 ...
 */
gcmCore.init = function(gameWindow, commonUIWindow, gcmWebServiceBaseUrl) {
  ...
  gcmCore.gameConfig_ = new GameConfig(gcmCore.configReady_, gcmWebServiceBaseUrl + '/config/');
  gcmCore.gameConfig_.setGameName(gcmCore.gcmBridge_.getGameName());
  gcmCore.gameConfig_.setPlayMode(gcmCore.gcmBridge_.getPlayMode());
  gcmCore.gameConfig_.setChannel(gcmCore.gcmBridge_.getChannel());
  //send gcm config request
  gcmCore.gameConfig_.init();

  //init game session web service
  gcmCore.gameSession_ = new GameSession(gcmWebServiceBaseUrl + '/session/',
                                          gcmCore.sessionTimerReady_, gcmCore.sessionStatsReady_);
  //TODO: uncomment this line on next sprint to enable session functions.
  //gcmCore.gameSession_.run();
 ...
 * done so, so that the game can enable it's UI.
 */
...
gcmCore.setGame = function(game) {
  ...
  //If config is ready, inform game config reference
  if (gcmCore.gameConfig_.isReady())
    gcmCore.game_.configReady();
};
...
{
  return gcmCore.account_.accountInit(accountInfo, balances);
};

/**
 * For simplicity we request only a height for now. In the future we may include
 * a range of options for height, width and position
 ...
 */
gcmCore.commonUIResize = function(height, width) {
  ...
  gcmCore.gcmBridge_.commonUIResize(height, width);
};
 ...
 * @param {string} errorMessage the error message provide by game.
 */
gcmCore.handleError = function(errorCategory, errorSeverity, errorCode, errorMessage)
{
  gcmCore.errorHandler_.handleError(errorCategory, errorSeverity, errorCode, errorMessage);
};


/**
 * This function is called by commonUI when it's done handling non-fog/RGI error.
 * gcm will call game.resume() to resume the game play.
 */
gcmCore.resume = function()
{
...
gcmCore.getConfig = function() {
  return gcmCore.gameConfig_.getConfig();
};
 ...
 * 'TURBO' and game display options such as 'PAYTABLE', 'ABOUT' and 'GAME_PREFERENCE'
...
gcmCore.gameAnimationStart = function() {
  GamePhaseController.gameAnimationStart();
};
...
gcmCore.gameAnimationComplete = function(resumeCallback) {
  GamePhaseController.gameAnimationComplete(resumeCallback);
};

/**
 * Both game and commonUI can call gcm.playForReal() to reload the game in real mode.
 * */
gcmCore.playForReal = function() {
  var currentURL = gcmCore.gameWindow_.location.href;
  gcmCore.gameWindow_.location.href = UrlUtil.updateSearchParameterByName('playMode', 'real', currentURL);
};

/**
 * PREVIEW - This will be called by common UI when user choose to continue on a game session
 * */
gcmCore.sessionContinue = function() {
  gcmCore.gameSession_.requestSessionContinue();
};

/**
 * PREVIEW - This will be called by common UI when user choose to quit the current game session
 * */
gcmCore.sessionQuit = function() {
...
goog.exportSymbol('com.openbet.gcm.sessionContinue', gcmCore.sessionContinue);
goog.exportSymbol('com.openbet.gcm.sessionQuit', gcmCore.sessionQuit);
